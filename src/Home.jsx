// Home.jsx (Updated to remove client-side verification/recording)
...
export default function Home({ externalActiveTab = 'menu', onTabChange, setGlobalCartOpen }) {
    ...
    const handleCheckout = async (totalAmount) => {
        ...
        try {
            ...
            // 1. Call server to create order (server pre-records and gets session ID)
            const response = await fetch(`${import.meta.env.VITE_CASHFREE_API_URL}/api/create-order`, {
                ...
                body: JSON.stringify({
                    amount: totalAmount,
                    currency: 'INR',
                    // Pass full cart data for server to pre-record in raw_cart_data column
                    cart: cartArray.map(ci => ({
                        item: ci.item, // Include the item object
                        qty: ci.qty,
            ...
            // 3. Post-Payment Client Cleanup (replaces old verification/recording)
            // The Webhook will handle the actual order recording on the server.
            setCart({});
            closeCart();
            alert('Payment initiated. Please check your "Orders" tab shortly for the final status.');
            onTabChange?.('orders');
