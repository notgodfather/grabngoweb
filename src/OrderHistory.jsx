...
export default function OrderHistory() {
  const profile = JSON.parse(localStorage.getItem('profile') || 'null'); // persist user locally [web:65]
  const userId = profile?.sub || ''; // guard [web:65]
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button
          onClick={() => { setLoading(true); fetchUserOrders(); }}
      ...
      {orders.length === 0 ? (
        <p style={{ marginTop: 12 }}>You haven't placed any orders yet.</p>
      ) : (
                  ...
                  {(o.order_items || []).map((r) => (
                    <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div style={{ width: 90, textAlign: 'right', fontWeight: 600 }}>
                        {formatPrice(Number(r.price || 0) * Number(r.qty || 0))}
                      </div>
                    </div>
                  ))}
                </div>
