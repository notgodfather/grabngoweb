...
export default function CartModal({ cart, onClose, onUpdateQuantity, onCheckout, isCheckingOut }) {
  ...
  // Calculate base total using original price only
  const total = cartArray.reduce((sum, ci) => {
    const originalPrice = Number(ci.item.price);
    return sum + originalPrice * ci.qty;
  }, 0);
  ...
  return (
    <div style={modalBackdropStyle} onClick={handleClose}>
      <div style={dynamicContentStyle} onClick={(e) => e.stopPropagation()}>
        <div
          style={{
            display: 'flex',
              ...
              return (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#0f172a', fontWeight: 700 }}>{formatPrice(Number(item.price))}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button onClick={() => onUpdateQuantity(item.id, -1)} style={quantityButtonStyle}>
                      −
                    </button>
